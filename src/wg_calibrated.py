...
class WGC(UMBSelect):
    ...
    def recalibrate(self):
        ...
        final_prev_point = np.repeat(-2, self.n_bins).reshape(self.n_bins)
        while low<=high:
            mid = (low + high)//2
                ...
                for r in range(l,self.n_bins):
                    # for k in range(l):
                        ...
                        if S[l][r] and dp[l-1]!=0 and dp[l-1]+1>dp[r]:
                            ...
                            prev_point[r] = l-1
                            # print(f"{l, r}")
            if prev_point[self.n_bins-1] != -2:
                final_dp = dp
                final_prev_point = prev_point
                alpha = mid
                high = mid - 1
            else:
                low = mid + 1
        ...
        return final_dp, final_prev_point,alpha/100

    def get_optimal_partition(self,r):
        assert (self.dp is not None and self.prev_point is not None), "not yet recalibrated"
    ...
    def fit(self, X_est, y_score, y, m, k):
        ...
        self.dp, self.prev_point,self.alpha = self.recalibrate()
        print(f"{self.alpha =}")
