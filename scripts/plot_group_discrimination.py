...
if __name__ == "__main__":
    ...
    metrics = ["discriminated_against", "group_num_in_bin", "bin_values",
               "group_num_positives_in_bin"]  # ,"log_loss","accuracy"]  #"alpha","accuracy"
    ...
    for z, Z_indices in enumerate(Z):
        ...
        for run in runs:
            for group in range(num_groups):
                results[group]["group_num_positives_in_bin"]["values"][run] = np.sum(
                    results[group]["group_num_in_bin"]["values"][run]) / the_n_cal
                results[group]["bin_values"]["values"][run] = np.sum(
                    results[group]["bin_values"]["values"][run] * results[group]["group_num_in_bin"]["values"][
                        run]) / np.sum(results[group]["group_num_in_bin"]["values"][run])
                results[group]["group_num_in_bin"]["values"][run] = np.sum(
                    np.where(results[group]["discriminated_against"]["values"][run], \
                             results[group]["group_num_in_bin"]["values"][run], \
                             np.zeros(results[group]["group_num_in_bin"]["values"][run].shape))) / np.sum(
                    results[group]["group_num_in_bin"]["values"][run])
        ...
        handles = []
        mean_group_bin_value = np.array(
            [results[group]["group_num_positives_in_bin"]["mean"] for group in range(num_groups)])
        args_sored = np.argsort(mean_group_bin_value)

        mean_algorithm = np.array([results[group]["group_num_in_bin"]["mean"] for group in range(num_groups)])[
            args_sored]
        std_algorithm = \
            np.array([results[group]["group_num_in_bin"]["std"] / np.sqrt(n_runs) for group in range(num_groups)])[
                args_sored]
        std_group_bin_value = np.array(
            [results[group]["group_num_positives_in_bin"]["std"] / np.sqrt(n_runs) for group in range(num_groups)])[
            args_sored]
